---
title: "Linux 원격 접속 네트워크: Telnet과 SSH"
...
## **네트워크 연결 확인**
---
Deskytop Rocky Linux 8에서 네트워크를 구성하기 위해 랜선을 사용하여 Ethernet 연결했습니다.

```bash
nmcli d
```
> **네트워크 인터페이스 enp6s0의 연결 상태 확인: connected**
{: .prompt-warning }
...
## **네트워크 구성**
---
Server와 Client는 같은 사설 네트워크에 위치해 있습니다. Server는 Desktop Rocky Linux OS를 사용하고 있고 Client는 Window11 OS를 사용하고 있습니다. 해당 글에서는 Client에서 Server로 원격 접속을 시도하는 상황으로 설정하였습니다.
...
### **1. tcpdump**
- **설명**:
    - CLI 환경에서 동작하는 Packet analzer
    - 서버의 특정 포트로 송수신되는 패킷 확인 가능
- **설치 명령어**
    ```bash
    yum install tcpdump 
    ```
- **예시 명령어**<br>
    **예시 1** enp6s0 인터페이스(이더넷 포트)를 통해 송수신 되는 패킷 캡쳐<br>
    ```bash
    tcpdump -i enp6s0
    ```
    **예시 2** ICMP 프로토콜 캡쳐<br>
    ```bash
    tcpdump -i enp6s0 icmp
    ```   
    **예시 3** 특정 Host에서 Port22로 오가는 TCP 패킷을 최대 1500 바이트 캡처하여 파일로 저장
    - `-w tcpdump.log`: 캡처한 패킷을 tcpdump.log 파일에 저장
    - `-s 1500` : 이더넷의 MTU 값 1500에 따라 캡쳐할 패킷의 스냅샷 길이 1500 바이트로 설정
...
- **사용 명령어**
    ...
    ```
- **예시 명령어**<br>
    **예시 1** 현재 firewalld 기본 zone 확인<br>
    ```bash
    firewall-cmd --get-default-zone
    ```
    **예시 2** 허용된 service 목록 확인<br>
    ```bash
    firewall-cmd --list-service
    ```
    **예시 3** 현재 firewalld 설정 정보 확인<br>
    ```bash
...
- **Telent**
    ...
    - **well-known Port**: 23
    - 데이터 전송 중 정보가 암호화되지 않아 보안에 취약
- **SSH(Secure Shell)**
    - Application Layer Protocol
    - **well-known Port**: 22
    - Telent에 보안 기능이 추가된 프로토콜
<br/><br/>
<br/><br/>
...
1. **Telent 서비스 추가**
- 허용할 서비스(telnet)가 지원하는 서비스 목록에 존재하는 지 확인
```bash
firewall-cmd --get-service
```
- public zone에 telnet 서비스를 추가
```bash
...
### **Window11에서 Telnet 클라이언트 사용** 
- **설정**: 제어판 > 프로그램 및 기능 > Windows 기능 켜기/끄기 > Telnet 클라이언트 옵션 선택
- **원격 접속**
